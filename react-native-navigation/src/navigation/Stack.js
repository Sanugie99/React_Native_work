...
const StackNavigation = () => {
    return(
        ...
        <Stack.Navigator 
            ...
            screenOptions={{
                cardStyle:{backgroundColor:'#fff'},//각 화면을 카드처럼 관리하겠다. 배경색, 패딩, 마진을 지정할 수 있다.
                headerStyle:{
                    height: 110, //헤더 높이
        ...
        >
            {/* name에는 화면의 이름을 작성하는데, Screen 컴포넌트의 name은 반드시 서로 다른 값을
            가져야 한다. */}
            ...
            <Stack.Screen 
                ...
                options={{
                    ...
                    headerLeft : () => { //안드로이드에서는 화면 왼쪽에 처리를 하기 위해는 headerLeft를 써야한다.
                        const navigation = useNavigation();// 현재 스크린의 navigation객체를 가져온다.
                        //함수형 컴포넌트 안에서 스크린 이동, 뒤로가기, 파라미터 전달 등 네비게이션
                        //관련 기능을 사용할 수 있다.
                        return(
                            //상단 왼쪽의 버튼
                            <Pressable
                                style={({pressed}) => ({
                                    marginLeft : 10, //아이콘 왼쪽 여백
