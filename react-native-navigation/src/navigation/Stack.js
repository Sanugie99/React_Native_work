...
//스택 네비게이션 객체를 생성
//Stack객체에 Stack.Navigator, Stack.Screen등의 컴포넌트가 포함되어있다.
const Stack = createStackNavigator();

const StackNavigation = () => {
    return (
        //여러 스크린을 관리하는 컨테이너이다.
        //내부에 <Stack.Screen> 컴포넌트를 정의하여 사용할 화면을 등록한다.
        <Stack.Navigator
            initialRouteName="Home" //첫번째로 띄울 화면을 지정
            //cardStyle을 이용해서 각 화면의 배경색을 설정
            screenOptions={{
                cardStyle: { backgroundColor: '#fff' },//각 화면을 카드처럼 관리하겠다. 배경색, 패딩, 마진을 적용
                headerStyle: {
                    height: 110,//헤더의 높이
                ...
                headerTitleStyle: { color: '#fff', fontSize: 24 },//타이틀 글자색, 글자 크기
        ...
        >
            {/* name에는 화면의 이름을 작성하는데, screen 컴포넌트의 name은 반드시 서로 다른 값을 가져와야한다. */}
            {/* name : 화면 고유의 이름, 나중에 화면을 이동할 때 필요한 이름이다.
                component : 연결된 실제 컴포넌트, 해당 name이 호출되면 해당 컴포넌트가 렌더링 된다. */}
            <Stack.Screen
                name="Home"
                component={Home}
                options={{ headerShown: false }} //홈화면은 헤더 숨김
            />
            <Stack.Screen
                name="List"
                component={List}
                options={{
                    ...
                    headerTintColor: '#e74c3c', //헤더 안 아이콘의 화살표 색상
                    headerLeft: () => { //안드로이드에서는 화면 왼쪽에 처리를 하기 위해서는 headerLeft를 써야한다.
                        const navigation = useNavigation(); //현재 스크린의 navigation 객체를 가져온다
