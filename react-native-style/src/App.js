    ...
    return (
        <ThemeProvider theme={isDark ? darkTheme : lightTheme}>
            <Container>
                ...
                {/* <Text
                style={[textStyles.text, {color:"green"}]}>
                Inline styling Text
            </Text>
            {/* 여러개의 스타일을 적용해야 하는 경우 배열을 이용하여 style 속성에 여러 개의 스타일을 적용하면 된다.
            뒤에 오는 스타일이 앞에 있는 스타일을 덮는다는 것을 기억해야 한다.
            여러개의 스타일을 적용할 때 반드시 클래스 스타일만 적용해야하는 것은 아니다
...
//인라인 스타일 방식으로 작성했을 때는 왜 color:"red"인지 코드만으로
//명확하게 이해하기 어려웠는데 StyleSheet를 사용하면 error라는 이름으로
//오류가 있는 상황에서 사용하기 위한 것이라는 의도를 파악하기 쉽다.
//글자색을 변경하려고 할 때 클래스 스타일 방식에서는 error 객체에서 색을 변경하면 되지만
//인라인 방식에서는 파일을 찾아 다니면서 전부 다 변경해야 한다.
